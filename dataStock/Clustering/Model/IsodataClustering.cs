...
namespace Giwer.dataStock.Clustering.Model
{
    ...
    public class IsodataClustering : CentroidBasedClustering
    {
        ...
        #region Constructors
        public IsodataClustering(GeoImageData gImgData, int[] bandsToUse,
        ...
        {
            InitParameters(initClusterNum, minClusterSize, pointsFromCentroidSDLimit, minCentroidDist, maxMergePerIter);
        }
        public IsodataClustering(byte[] band,
            uint maxIter = MaxIterDefaultVal,
            uint initClusterNum = InitClusterNumDefaultVal, 
            uint maxClusterNum = MaxClusterNumDefaultVal,
            uint minClusterSize = MinClusterSizeDefaultVal,
        ...
        private void InitParameters(
            uint initClusterNum, 
            uint minClusterSize,
            float pointsFromCentroidSDLimit,
            float minCentroidDist,
            uint maxMergePerIter)
        {
        ...
        private void OnIterationStepTaken()
        {
            ...
            OnExecutionProgressUpdated(CurrentIter);
        }
        #endregion
        ...
        private List<int[]> DimwiseDiffsFromCentroid(Cluster cluster)
        {
            ...
            {
                int[] dimwiseDiff = Point.DimwiseDiff(point, cluster.Centroid);
                res.Add(dimwiseDiff);
            }
            return res;
        }
        ...
        {
            Point offset = SplittingCentroidOffset(maxSDDim, maxSDSquared);
        ...
        private Point SplittingCentroidOffset(int maxSDDim, float maxSDSquared)
        {
            byte modifier = (byte)(maxSDSquared * SplittingCentroidFactor);

            Point offset = new Point(PointDim);
            offset[maxSDDim] = modifier;

            return offset;
        }

        private (CentroidPoint, CentroidPoint) SplittingCentroidsFor(Cluster cluster, Point offset)
        {
            CentroidPoint ctr1 = CentroidPoint.CentroidOfValues(cluster.Centroid);
            ctr1.Add(offset);

            CentroidPoint ctr2 = CentroidPoint.CentroidOfValues(cluster.Centroid);
            ctr2.Subtract(offset);

            return (ctr1, ctr2);
        }

        private void CreateSplitClusterFrom(CentroidPoint newCentroid)
        {
            _clusters[newCentroid.ID] = new Cluster(newCentroid);
        }
        #endregion
        ...
        private List<(uint, uint, float)> FilteredCentroidDists()
        {
            ...
            for (int i = 0; i < _clusters.Count; i++)
            {
                ...
                {
                    CentroidPoint ctr1 = _clusters.ElementAt(i).Value.Centroid;
                    CentroidPoint ctr2 = _clusters.ElementAt(j).Value.Centroid;
        ...
        private void AddPairDistBelowThresholdTo(List<(uint, uint, float)> dists, CentroidPoint ctr1, CentroidPoint ctr2)
        {
            float dist = Point.SquaredEuclDist(ctr1, ctr2);
