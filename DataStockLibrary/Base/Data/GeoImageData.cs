...
namespace DataStockLibrary.Base.Data
{
    public class GeoImageData
    {
        ...
        public enum GeoImageType
        {
            [field: Description("Giwer Header")]
            GWH,
            [field: Description("Bil Header")]
            BIL,
            [field: Description("BSQ Header")]
            BSQ,
            [field: Description("GeoTiff")]
            TIF,
            [field: Description("JPEG")]
            JPG,
        ...
        public string FileName
        {
            ...
            set
            {
                ...
                parseHeader();
                computeStride();
                if (_fileType == GeoImageType.BIL || _fileType == GeoImageType.ENVI) { _bytesPerPixel = (_nBits / 8) * _nBands; }
            }
        }

        GeoImageType _fileType;
        public GeoImageType FileType
        {
        ...
        {
            _fileType = FileHelper.GetImageType(Path.GetExtension(_fileName).ToUpper());
            if (_fileType.Equals(GeoImageType.BIL))
            {
                _fileType = FileHelper.GetImageType(_fileName, "ENVI", GeoImageType.ENVI);
                _comment = "This is an BIL file";
            }
            else if (_fileType.Equals(GeoImageType.BSQ))
            {
                _fileType = FileHelper.GetImageType(_fileName, "ENVI", GeoImageType.ENVI);
        ...
        void parseHeader()  // parse header content from any image format which is supported by Giwer
        {
            switch (_fileType)
            {
                ...
                case GeoImageType.JPG:
                    ...
                    parseJPGParams();
                    return;
                case GeoImageType.DDM:
                    resetImageParameters();
                    parseDDMParams();
                    return;
                case GeoImageType.GWH:
                    resetImageParameters();
                    parseGiwerHeader();
                    return;
                default:
